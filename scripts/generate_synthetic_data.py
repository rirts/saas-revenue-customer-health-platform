...
def build_accounts(fake: Faker, rng: Random) -> pd.DataFrame:
    ...
    for i in range(1, N_ACCOUNTS + 1):
        ...
        employee_band = {
            "SMB": weighted_choice(
                rng, ["1-10", "11-50", "51-100"], [0.25, 0.55, 0.20]
            ),
            "Mid-Market": weighted_choice(
                rng, ["101-250", "251-500", "501-1000"], [0.40, 0.40, 0.20]
            ),
        ...
        rows.append(
            {
                ...
                "industry": weighted_choice(
                    rng, industries, [0.16, 0.12, 0.16, 0.10, 0.11, 0.10, 0.12, 0.13]
                ),
                "segment": segment,
                "employee_band": employee_band,
                "country": weighted_choice(
                    rng, countries, [0.38, 0.25, 0.10, 0.10, 0.07, 0.10]
                ),
            ...
            rows.append(
                {
                    ...
                    "role": weighted_choice(
                        rng, roles, [0.25, 0.12, 0.14, 0.20, 0.18, 0.11]
                    ),
...
def build_deals(accounts: pd.DataFrame, rng: Random) -> pd.DataFrame:
    ...
    for i, account in enumerate(accounts.itertuples(index=False), start=1):
        ...
        created_at = pd.Timestamp(account.created_at) + pd.Timedelta(
...
def build_billing(
    accounts: pd.DataFrame, deals: pd.DataFrame, rng: Random
    ...
    for deal in won_deals.itertuples(index=False):
        ...
        start_date = pd.Timestamp(deal.close_date) + pd.Timedelta(
        ...
        status = weighted_choice(
            rng, ["active", "past_due", "canceled"], [0.76, 0.12, 0.12]
        )
        ...
        while current_month <= end_month:
            ...
            paid_at = (
                due_date + pd.Timedelta(days=rng.randint(-5, 10)) if is_paid else None
            )
                ...
                payments.append(
                    {
                        ...
                        "payment_method": weighted_choice(
                            rng, ["card", "bank_transfer", "wire"], [0.62, 0.28, 0.10]
                        ),
                    }
                )
                payment_id += 1
...
def build_product_events(subscriptions: pd.DataFrame, rng: Random) -> pd.DataFrame:
    ...
    for sub in subscriptions.itertuples(index=False):
        start = pd.Timestamp(sub.start_date)
            ...
            pd.Timestamp(sub.canceled_at)
            if isinstance(sub.canceled_at, str) and sub.canceled_at
            else END_DATE
        )
        ...
        if sub.status == "active":
            activity_multiplier = weighted_choice(
                rng, ["low", "medium", "high"], [0.25, 0.50, 0.25]
            )
        elif sub.status == "past_due":
            activity_multiplier = weighted_choice(
                rng, ["low", "medium", "high"], [0.55, 0.35, 0.10]
            )
        else:
            activity_multiplier = weighted_choice(
                rng, ["low", "medium", "high"], [0.70, 0.25, 0.05]
            )

        monthly_events = {
            "low": rng.randint(2, 8),
            "medium": rng.randint(9, 25),
            "high": rng.randint(26, 60),
        }[activity_multiplier]
            ...
            rows.append(
                {
                    ...
                    "event_type": weighted_choice(
                        rng, event_types, [0.45, 0.22, 0.12, 0.06, 0.10, 0.05]
                    ),
    ...
    for sub in subscriptions.itertuples(index=False):
        ...
        for _ in range(n_tickets):
            ...
            if status in {"resolved", "closed"}:
                ...
                resolved_at = (
                    created_at + pd.Timedelta(days=resolution_days)
                ).isoformat()
            ...
            rows.append(
                {
                    ...
                    "category": weighted_choice(
                        rng, categories, [0.18, 0.20, 0.22, 0.18, 0.10, 0.12]
                    ),
                }
            )
            ticket_id += 1
...
def main() -> None:
    ...
    billing_customers, subscriptions, invoices, payments = build_billing(
        accounts, deals, rng
    )
